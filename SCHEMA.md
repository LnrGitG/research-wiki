...
- **Two logs, split by kind of change:**
  - `log.md` — **содержательные** изменения: новые публикации (`ingest`),
    ...
    в знание*.
  - **технический журнал** — отдельный, в публичной витрине
    не публикуется: инфраструктура, базы данных, миграции, скрипты,
    CI, настройка сервисов. Правило: запись описывает, *как устроен
    инструмент*.

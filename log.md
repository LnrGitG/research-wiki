...
## 2026-09-17 — Проверка критериев приёмки
...
- Критерий 4: из 52 таблиц каталога 29 существуют, 23 отсутствуют. Разбор: ЕИСЖС и ДОМ.РФ представлены типами внутри domrf_indicators (857 862 строки, 11 data_type), а не отдельными таблицами; остальные действительно не собраны.
- Результат: queries/acceptance-criteria-check.md

## 2026-09-17 — Гармонизация: рубежи 1-2
...
- Рубеж 2: 483 метрики в core.metric. Коды — из готовых кодов источников (panel: y477030001) либо первые буквы названия после транслитерации (pkzznns). Проверка: 483/483 уникальны, 0 без единицы или частоты.
...
## 2026-09-17 — Гармонизация: рубеж 3 (наблюдения)
- Восстановлен data/region_aliases.csv (444 записи) из тега backup/regions-before-cleanup-20260917 — файл был пуст.
- Записано 432 алиаса в meta.region_alias и 6 релизов в core.release (по одному на источник).
- Покрытие регионов: ЦБ/panel/ввод/сделки — 100%, ИКВ/индексы/цены — 99%.
- Нерегиональные подписи (годы, кварталы, виды работ — 93 строки) уходят в sub_dimension с регионом РФ.
- Исправлен баг сопоставления метрик: ключ нормализовался, а словарь строился по сырому названию — терялись 13 103 строки.
- scripts/harmonize_3a_aliases_releases.py, scripts/harmonize_3b_observations.py.
- Туннель к PostgreSQL переведён в systemd-юнит pg-tunnel.service (переживает перезапуск шлюза).

## 2026-09-17 — Гармонизация: рубеж 3 завершён (551 022 наблюдения)
...
- Устранено: OOM на VDS (перенос выполнен на ВМ с потоковым чтением), конфликт коммита с серверным курсором (разные соединения), баг нормализации ключей (терялось 13 103 строки), гомоглифы в подписях регионов.
- Туннель переведён в systemd-юнит pg-tunnel.service.

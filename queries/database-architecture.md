...
CREATE TABLE core.observation (
    ...
    CONSTRAINT uq_observation UNIQUE (metric_id, region_id, frequency_id, period_start,
                                      source_id, release_id, assessment_type),
...
## 14. Открытые вопросы (требуют решения владельца)

1. **Стоимость постоянной ВМ** (~2 600–3 300 ₽/мес против ~200 ₽ у прерываемого
   сборщика). Альтернатива — держать PostgreSQL на существующем VPS (но там 4,5 ГБ
   свободно — недостаточно) либо согласовать бюджет. Нужен явный OK на постоянную ВМ
   или решение перенести базу на VPS с расширением диска.
2. **Судьба фантомных таблиц**: какие из 23 действительно планируются к сбору
   (например, `cbr_usd_rate` — курс ЦБ легко получить), а какие — удалить из
   каталога навсегда. Требуется ревью владельцем списка из §12.
3. **Канонический справочник регионов**: маппинг 339 вариантов требует ручных
   решений по ~40 неоднозначным кейсам (архаичные АО, «без авт. округа», слияния
   2005–2008). Кто утверждает карту соответствий — агент с последующим ревью или
   владелец целиком?
4. **Разрывы рядов (`series_comparability`)**: link-коэффициенты для ИФО
   (2011→2021) и ВДС/ИКВ (ОКВЭД1→ОКВЭД2) должны быть вычислены и утверждены до
   того, как derived-слоя начнёт «сшивать» ряды. Источник коэффициентов — Росстат
   публикует сопоставимые ряды; нужно решение, брать методологию Росстата или
   считать свою сцепку.
5. **Строгость ключа `uq_observation`**: допускаем ли мы, что один источник в одном
   релизе публикует одну оценку одного периода (текущий дизайн) — или возможны
   легитимные дубли внутри релиза (например, разные `row_label` в Росстате)?
   Если да — в ключ добавляется колонка `sub_dimension` (row_label).
6. **Grep-чистота от GCS**: удалять ли упоминания `gs://` из *исторических* вики-страниц
   (log.md, отчёты), или ограничить критерий только кодом и конфигурацией?
7. **Схема гранулярности ФНС firm-level** (3,7 млн строк «Точно-ст», по фирме × год):
   держать ли фирменный уровень в `core.observation` (регион-агрегаты через
   `region_id`, но firm-разрез требует отдельной таблицы/схемы) или вынести в
   отдельную схему `firms` вне observation-домена. Рекомендация — отдельная схема,
   но нужно подтверждение, что firm-level панель будет использоваться.
8. **Retention staging-слоя**: сколько релизов держать в staging перед очисткой
   (предложено: N=3 последних + все за последний месяц).
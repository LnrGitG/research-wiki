...
EXACT = {
 ...
 "архангельская область (с автономным округом)": "ARCTIC_MERGE_NAO",  # спецслучай
 ...
 "тюменская область (с автономными округами)": "TYUMEN_MERGE_AO",
 ...
 # прочее
 "малороссийский фо": "NONEXISTENT_FD",  # ошибочное/устаревшее название, на ревью
 ...
 "сочи": "SOCHI_CITY",  # город, не субъект — на ревью
...
def normalize(s: str) -> str:
    ...
    # убрать скобочные уточнения, дублирующие регион: «(Татарстан)», «(Адыгея)»
    s = re.sub(r'\s*\(([^)]*)\)\s*$', lambda m: ' ' + m.group(1) if 'татарстан' in m.group(1).lower() or 'адыгея' in m.group(1).lower() or 'чувашия' in m.group(1).lower() or 'якутия' in m.group(1).lower() else '', s)
...
# ──────────────────────────────────────────────────────────────────────
NON_REGION_PATTERNS = [
 (r'^(продано|не продано|продажи не открыты|продано, %|продано, тыс\. кв\. м)$', 'row_label_sales'),
...
for name, cnt in raw.items():
    ...
    if code and not code.isupper():
        canon = REGIONS.get(code, (FD.get(code, ""),))[0] if code not in FD else FD[code]
        alias_rows.append({
            "raw_name": name, "count": cnt, "region_code": code,
            "canonical_name": canon,
            "level": "federal_district" if code in FD else ("country" if code == "ru" else "region"),
...
with open(DATA/"region_aliases.csv","w",newline="",encoding="utf-8") as f:
    w = csv.DictWriter(f, fieldnames=["raw_name","count","region_code","canonical_name","level","method","sources"])
    w.writeheader(); w.writerows(sorted(alias_rows, key=lambda r:-r["count"]))

canon_rows = [{"region_code":"ru","name":"Российская Федерация","level":"country","okato":"","parent_code":""}]
canon_rows += [{"region_code":c,"name":n,"level":"federal_district","okato":"","parent_code":"ru"} for c,n in FD.items()]
canon_rows += [{"region_code":c,"name":n,"level":"region","okato":o,"parent_code":p}
               for c,(n,o,p) in REGIONS.items()]
with open(DATA/"regions_canonical.csv","w",newline="",encoding="utf-8") as f:
    w = csv.DictWriter(f, fieldnames=["region_code","name","level","okato","parent_code"])

...
рынок, валюту, вид ряда, продукт.

ВАЖНО про колонку `source` внутри staging-таблицы: это ПЕРВИЧНАЯ ПУБЛИКАЦИЯ
(сборник Росстата против бюллетеня ЦБ, годовой файл против месячного), и её
место — в release_id. Если не различать публикации, одинаковые значения разных
выпусков попадают в один ключ и задваиваются при агрегации. Перед прогоном
смотреть `SELECT count(DISTINCT source) FROM таблица`. Разбор — в
queries/harmonization-stage5-source-fix.md.

Идемпотентность: строки прошлого прогона удаляются по тегу stage5 у метрик
(обойти очистку — переменная окружения SKIP_CLEAN=1).

Перед прогоном — симуляция ключа.
